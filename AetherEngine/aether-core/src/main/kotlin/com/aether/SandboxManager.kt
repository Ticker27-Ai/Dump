...
object SandboxManager {
 ...
 /** PGL version hash — ตรงกับ blueprint the reference engine (90d8aa15a2de2cb4) */
    ...
    // [CUT 2026-09-23] CONFIG_ENDPOINT (rest.snakeseller.com/api/request/)
    // — endpoint C2/license จุดเดียวของฝั่ง Kotlin ถูกถอดทั้งก้อนพร้อม
    // fetchRemotePglMap/fetchRemoteVersion — เหลือ offline 100%
    // (ห้ามเพิ่ม endpoint กลับมาโดยไม่ผ่าน docs/CUTS.md)

    // ---- FLEXIBLE VERSION MAP (offline — ตาราง DEFAULT_* ในไฟล์นี้) ----
    // resolvePglVersions() เลือก PGL hash จาก (1) versionCode ของเครื่อง
    // (2) อ่าน dir จริง (3) ตาราง DEFAULT_* — ไม่มีการดึงผ่านเน็ต
    // ([CUT 2026-09-23] ตัด CONFIG_ENDPOINT + fetchRemotePglMap แล้ว)
    ...
    // [CUT 2026-09-23] fetchRemotePglMap — ดึง map ผ่าน CONFIG_ENDPOINT
    // (rest.snakeseller.com) ถูกถอดทั้งฟังก์ชัน (นโยบาย offline ดู docs/CUTS.md)
 ...
 /** j9g29zqf0cfqd3vvu2bw — profile cache dir (จาก data dump) */
    ...
    fun resolvePglVersions(targetPkg: String = this.targetPkg): List<String> {
        ...
        // 3. ตาราง DEFAULT_* ในไฟล์นี้ (offline — [CUT 2026-09-23] ตัด
        //    fetchRemotePglMap + CONFIG_ENDPOINT ทิ้งแล้ว)
 ...
 // 1. package.conf — Manifest Snapshot (Binary format)
    ...
    fun generatePackageConf(target: String = targetPkg): ByteArray {
        ...
        return try {
            ...
            // [CUT 2026-09-23] fetchRemoteVersion (CONFIG_ENDPOINT) ถูกถอด —
            // ใช้ versionName บนเครื่องอย่างเดียว (offline 100%)
            val resolvedVersion = pi.versionName
