 ...
 * 2. fetchAsync() — offline (cache → fallback) → notify listeners
...
object RemoteConfig {
    ...
    /** ชื่อเก่า (compat) — ไม่มี remote จริง ใช้ fetchAsync() */
    @Deprecated("offline-only — ใช้ fetchAsync()", ReplaceWith("fetchAsync()"))
    fun fetchRemoteAsync() = fetchAsync()

    fun fetchSync(): RemoteConfig? {
        return loadCache() ?: offlineFallback()
    }

    /** ชื่อเก่า (compat) — ไม่มี remote จริง ใช้ fetchSync() */
    @Deprecated("offline-only — ใช้ fetchSync()", ReplaceWith("fetchSync()"))
    fun fetchRemoteSync(): RemoteConfig? = fetchSync()
