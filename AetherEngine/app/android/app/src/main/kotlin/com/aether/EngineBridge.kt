...
object EngineBridge : MethodCallHandler {
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        try {
            when (call.method) {
                ...
                "getInstalledGameInfo" -> result.success(getInstalledGameInfo(call.argument<String>("packageName")))
    ...
    private fun getDeviceId(): String {
        ...
        return try {
            ...
            Log.w(TAG, "getDeviceId failed: ${e.message}")
            "aether-android"
        }
    }

    private fun getInstalledGameInfo(packageName: String?): Map<String, Any>? {
        val c = ctx ?: return null
        if (packageName.isNullOrEmpty()) return null
        return try {
            val info = c.packageManager.getPackageInfo(packageName, 0)
            mapOf(
            ...
            Log.w(TAG, "getInstalledGameInfo($packageName) failed: ${e.message}")
            null
        }
    }
